...
public abstract class Shop extends Unit implements ShopInterface {
	Shop() {super();}
	Shop(String name) {
		super(name);
		Utils.debugPrintln("Shop: Winkel \"" + getName() + "\" is aangemaakt en is van het type [" + this.getClass().getSimpleName() + "]");
	...
	public String getSecurityInfo(String indentationString) {
		return indentationString + Security.checkSecurity() + "\n" + 
	           indentationString + "[" + this.getClass().getSimpleName() + "] Security niveau is van het type winkel.";
	}
}
